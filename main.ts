...
console.log(`  TP       : ATR-adaptive (${process.env.TP_ATR_MULT ?? '1.0'}× live 5m ATR, floor=$${process.env.TP_MIN_USD ?? '0.50'}, ceil=$${process.env.TP_MAX_USD ?? '15.00'}), post-only maker, 0 fee`);
console.log(`  SL       : DERIVED live so (loss + taker fee) == ${_slMult}x TP when TP is large enough that fee is negligible; FLOATS ABOVE ${_slMult}x on small (calm-market) TPs since the fixed-$ fee dominates then`);
console.log(`  BREAKEVEN: ~${(100*Number(_slMult)/(1+Number(_slMult))).toFixed(1)}% WR when the ${_slMult}x cap holds; higher on small-TP trades where the ratio floats (frequency prioritized over an exact cap)`);
