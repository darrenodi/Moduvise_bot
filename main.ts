...
console.log(`  SL       : DERIVED so (loss + taker fee) == ${_slMult}x TP when TP is large enough that fee is negligible; at the fixed $0.50 TP the fee dominates, so SL floors at TP itself (~1:1 price distance, ~4.3x realized loss incl. fee)`);
console.log(`  BREAKEVEN: ~${(100*Number(_slMult)/(1+Number(_slMult))).toFixed(1)}% WR if the ${_slMult}x cap held; realized bar is closer to ~81% at the fixed $0.50 TP (fee-dominated regime)`);
