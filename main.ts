...
console.log(`  TP1      : ATR×${process.env.TP_ATR_MULT ?? '0.10'} (min $${process.env.TP_MIN ?? '0.05'}, max $${process.env.TP_MAX ?? '1.00'})`);
console.log(`  TP2      : rescue limit after 90s`);
console.log(`  SCRATCH  : market exit after 120s`);
console.log(`  ATR GATE : $${process.env.ATR_CEILING ?? '6.00'} max — sits out trap markets`);
